...
def apply_pub_style() -> None:
    ...
    plt.rcParams.update({
        ...
        'legend.fontsize': 10, 'figure.titlesize': 15, 'figure.autolayout': True,
    })


def plot_confusion_matrices(test_predictions, results, y_test, out_dir) -> str:
    import matplotlib.pyplot as plt
    ...
    path = os.path.join(out_dir, "figure1_confusion_matrices.png")
    plt.savefig(path, dpi=300)
    plt.close(fig)
    return path


def plot_roc_pr_curves(test_predictions, y_test, out_dir) -> str:
    import matplotlib.pyplot as plt
    ...
    path = os.path.join(out_dir, "figure2_roc_pr_curves.png")
    plt.savefig(path, dpi=300)
    plt.close(fig)
    return path


def plot_benchmark_barchart(df: pd.DataFrame, test_predictions, out_dir) -> str:
    import matplotlib.pyplot as plt
    ...
    path = os.path.join(out_dir, "figure3_benchmark_barchart.png")
    plt.savefig(path, dpi=300)
    plt.close()
    return path


def generate_all_figures(test_predictions: Mapping[str, np.ndarray], results,
                         y_test: np.ndarray, df: pd.DataFrame,
                         out_dir: str, verbose: bool = True) -> Mapping[str, str]:
