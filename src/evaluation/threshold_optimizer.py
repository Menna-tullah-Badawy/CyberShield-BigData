...
class ThresholdOptimizer:
    """واجهة كلاس لمحسّن العتبة الأمنية (نفس الدالة بالنص)."""

    def __init__(self, target_prec: float = TARGET_PRECISION,
                 target_rec: float = TARGET_RECALL):
        self.target_prec = target_prec
        self.target_rec = target_rec

    def find_optimal_threshold(self, y_true, y_probs, val_true, val_probs):
        return optimize_security_threshold(
            y_true, y_probs, val_true, val_probs,
            target_prec=self.target_prec, target_rec=self.target_rec)
