"""
Robust Generalization & IEEE/ACM Benchmark Diagnostic Suite.
...
def classify_generalization_status(
    gap_roc: float, gap_f1: float, roc_te: float, roc_tr: float, prec_te: float
) -> str:
...
def run_diagnostics(
    ...
    """
    تشخيص التعميم المتزامن (Train vs Test) + حفظ CSV + رسم المقارنة.
    models: قاموس الموديلات المدربة (Tabular + DL + يُبنى الـ Ensemble داخلياً).
    """
    ...
    for name, model in models.items():
        ...
        if hasattr(model, "predict_proba"):  # Tabular
    ...
    # تقييم الـ Hybrid Ensemble (نفس الأوزان)
    from src.nids.ensemble import HYBRID_WEIGHTS
    w = HYBRID_WEIGHTS
    ...
    for name, d in eval_data.items():
        ...
        diagnostic_rows.append({
            ...
            "Status": classify_generalization_status(gap_roc, gap_f1, roc_te, roc_tr, prec_te),
        })
    ...
    csv_diag_path = os.path.join(out_dir, DIAG_CSV)
    df_diag.to_csv(csv_diag_path)
    if verbose:
        print(f"\n📁 Saved Report: {csv_diag_path}")

    # الرسم البياني لمقارنة التعميم
