"""
Professional 7-page SOC Incident PDF Report.
نقل حرفي من Cell 4 في cybershield.ipynb (نفس الصفحات والألوان).

تحسين واحد متوافق: صفحة مقارنة الموديلات كانت بأرقام ثابتة في النوت بوك،
هنا تُبنى من نتائج البنش مارك الحقيقية إن توفرت (وتقع على القيم الأصلية غير ذلك).
"""
...
# القيم الثابتة الأصلية من النوت بوك (Fallback فقط)
...
def _comparison_arrays(results: Optional[Mapping[str, Mapping[str, float]]]):
    """مصفوفات صفحة المقارنة: من النتائج الحقيقية أو القيم الأصلية."""
    if results:
...
def generate_soc_report(
    ...
    cm: np.ndarray,
    xai_features: list,
    threat_types_detected: Mapping[str, dict],
    threat_kb: list,
    port: int,
    ...
    """توليد تقرير SOC المكوّن من 7 صفحات (نفس كود النوت بوك)."""
    import matplotlib
    matplotlib.use('Agg')
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        a.add_patch(mpatches.FancyBboxPatch((0.1, 0.2), 0.8, 0.6,
                    ...
                    edgecolor=C_CYAN, linewidth=2))
        a.text(0.5, 0.65, f'{tp + fp}', fontsize=48, ha='center', va='center',
               color=C_RED, fontweight='bold')
        a.text(0.5, 0.30, 'Threats Detected', fontsize=14, ha='center', va='center', color=C_WHITE)
        a.text(0.5, 0.15, f'out of {n_samples} samples', fontsize=10, ha='center', color=C_GRAY)

        a = axes[0, 1]
        a.add_patch(mpatches.FancyBboxPatch((0.1, 0.2), 0.8, 0.6,
                    boxstyle="round,pad=0.05", facecolor=C_DARK,
                    edgecolor=C_GREEN, linewidth=2))
        a.text(0.5, 0.65, f'{roc:.4f}', fontsize=48, ha='center', va='center',
               color=C_GREEN, fontweight='bold')
        a.text(0.5, 0.30, 'ROC-AUC Score', fontsize=14, ha='center', va='center', color=C_WHITE)
        a.text(0.5, 0.15, 'Discrimination Ability', fontsize=10, ha='center', color=C_GRAY)

        a = axes[1, 0]
        a.add_patch(mpatches.FancyBboxPatch((0.1, 0.2), 0.8, 0.6,
                    boxstyle="round,pad=0.05", facecolor=C_DARK,
                    edgecolor=C_PURPLE, linewidth=2))
        a.text(0.5, 0.65, f'{len(threat_types_detected)}', fontsize=48, ha='center', va='center',
               color=C_PURPLE, fontweight='bold')
        a.text(0.5, 0.30, 'Threat Types Identified', fontsize=14, ha='center',
               va='center', color=C_WHITE)
        a.text(0.5, 0.15, f'from {len(threat_kb)} MITRE categories',
        ...
        a.add_patch(mpatches.FancyBboxPatch((0.1, 0.2), 0.8, 0.6,
                    boxstyle="round,pad=0.05", facecolor=C_DARK,
                    edgecolor=C_YELLOW, linewidth=2))
        a.text(0.5, 0.65, f'{fnr * 100:.1f}%', fontsize=48, ha='center', va='center',
               color=C_YELLOW, fontweight='bold')
        a.text(0.5, 0.30, 'False Negative Rate', fontsize=14, ha='center',
               va='center', color=C_WHITE)
        a.text(0.5, 0.15, f'{fn} attacks missed', fontsize=10, ha='center', color=C_GRAY)
        ...
        ax2 = axes[1]
        im = ax2.imshow(cm, cmap='Blues', aspect='auto')  # noqa: F841
        ax2.set_xticks([0, 1])
        ax2.set_yticks([0, 1])
        ...
        fig.suptitle('Model Performance Comparison', fontsize=14,
                     fontweight='bold', color=C_DARK)
        models, recall, f1, roc_auc = _comparison_arrays(results)
        ...
        for tid, info in threat_types_detected.items():
            ...
            ax.text(0.06, y_pos - 0.075, "iptables:", fontsize=7, color=C_YELLOW, fontweight='bold')
