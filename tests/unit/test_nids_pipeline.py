"""
Unit tests for the src/nids Kaggle-track pipeline (ported from cybershield.ipynb).
...
def test_create_day_sequences_shape_and_vote():
    from src.nids.temporal_dataset import create_day_sequences

    X = np.random.rand(25, 4).astype(np.float32)
    y = np.array([1] * 10 + [0] * 15)
    Xs, ys = create_day_sequences(X, y, seq_len=10)
    assert Xs.shape == (2, 10, 4)
    # seq0 mean=1.0 → 1 ; seq1 has 0 attack rows in first 10? rows10-19 all 0 → 0
    assert list(ys) == [1, 0]


def test_strict_exclude_filters_identity(tmp_path):
    from src.nids.temporal_dataset import process_day_csv
    ...
    (x_tr, y_tr), (x_va, y_va), (x_te, y_te), cols, atk = process_day_csv(str(p))
    low = [c.lower() for c in cols]
    assert "src ip" not in low and "dst port" not in low and "protocol" not in low
    assert x_tr.shape[1] == 10 and atk == 10  # 50 attacks / SAMPLE_STEP 5


def test_threshold_optimizer_keys_and_bounds():
    from src.nids.threshold import optimize_security_threshold
...
def test_hybrid_ensemble_math():
    from src.nids.ensemble import HYBRID_WEIGHTS, hybrid_ensemble_predict

    assert abs(sum(HYBRID_WEIGHTS.values()) - 1.0) < 1e-9
    preds = {k: np.full(10, 0.5) for k in HYBRID_WEIGHTS}
    out = hybrid_ensemble_predict(preds)
...
def test_generalization_status_rules():
    from src.nids.diagnostics import classify_generalization_status

    assert "Ideal" in classify_generalization_status(0.02, 0.03, 0.90, 0.92, 0.95)
    assert "SOTA" in classify_generalization_status(0.10, 0.20, 0.87, 0.97, 0.91)
    assert "Drift" in classify_generalization_status(0.14, 0.20, 0.83, 0.97, 0.80)
    assert "Overfitting" in classify_generalization_status(0.30, 0.30, 0.60, 0.90, 0.50)


def test_balance_and_scale():
    from src.nids.benchmark import balance_train_1_to_4, scale_sequences

    rng = np.random.RandomState(3)
    X = rng.rand(500, 10, 6).astype(np.float32)
    y = (rng.rand(500) > 0.85).astype(np.int64)
    Xb, yb = balance_train_1_to_4(X, y)
    pos, neg = int(yb.sum()), int((yb == 0).sum())
    assert neg == min((y == 0).sum(), pos * 4)
    X3, Xv3, Xt3, _ = scale_sequences(Xb, X[:40], X[40:80])
...
def test_xai_importance_sums_to_one():
    ...
    feats = extract_xai(m, x.clone(), ["a", "b", "c", "d"], n=3)
    assert len(feats) == 3
